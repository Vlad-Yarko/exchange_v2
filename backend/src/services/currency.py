...
class CurrencyService(Service):
    ...
    @transaction
    async def subscribe_create_one(self, data: CurrencySubscribeBody) -> Union[dict, tuple[int, str]]:
        symbol = data.get("symbol1") + data.get('symbol2')
    ...
    @transaction
    async def subscribe_update_one(self, symbol: str, data: CurrencySubscribeBody) -> Union[dict, tuple[int, str]]:
        symbol = data.get("symbol1") + data.get('symbol2')
        d = await self.crypto_repo(self.session).get_one_by_symbol(symbol)
        if d:
            return (422, "Symbols combination has already found")
